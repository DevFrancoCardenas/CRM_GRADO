...
{ name: "Carrera", field: "nombre_carrera" },
//endJSnombre_carrera
//headers
];
function Carreras_list() {
...
//foreigns
    ...
    useEffect(() => {
        var temp = {};
        temp.read = store.checkPermissions(["read_carreras"]);
        temp.report = store.checkPermissions(["report_carreras"]);
        temp.update = store.checkPermissions(["update_carreras"]);
        temp.delete = store.checkPermissions(["delete_carreras"]);
            ...
            <div className="px-2  py-1 sm:py-2 xl:py-4 ">
                <div className="bg-gray-50 px-1  py-0.5 sm:py-0.5 2xl:py-1  sm:flex flex flex-wrap-reverse sm:items-center w-full">
                    <div className="sm:flex-auto">
                        <h1 className="text-balance 2xl:text-xl sm:text-lg text-sm  mt-2 sm:mt-0 font-semibold text-gray-900 uppercase">Lista de Carreras</h1>
                    </div>
                    <div className="ml-6 sm:ml-0 flex-none">
                        {store.checkPermissions(['create_carreras']) &&
                            <Link
                                to="../carreras/create"
                                type="button"
                    ...
                    <div className="inline-block max-w-full min-w-full align-middle">
                        <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-4 py-5 sm:flex-nowra">
                            <div className="flex items-center">
                                {store.checkPermissions(['report_carreras']) &&
                                    <Link
                                        to="../carreras/report"
                                        type="button"
                                        className="inline-flex items-center justify-center rounded-md border border-transparent bg-green-600 2xl:px-4 lg:px-2  py-2 2xl:text-sm sm:text-xs text-xs font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:w-auto"
...
<td className="2xl:px-3 sm:px-2 px-1 2xl:py-4 sm:py-2 py-1 2xl:text-sm text-xs text-gray-500">{carreras?.unidad_academica?.nombre_unidad_academica}</td>
{/* endJSXunidad_academica */}
{/* initJSXnombre_carrera */}
<td className="2xl:px-3 sm:px-2 px-1 2xl:py-4 sm:py-2 py-1 2xl:text-sm text-xs text-gray-500">{carreras?.nombre_carrera}</td>
{/* endJSXnombre_carrera */}
{/*fields*/}
                                                <td className="absolute py-4 pl-3 pr-4 text-sm font-medium">
                                                    <OptionsRUD id={carreras._id} model="carreras" permissions={permissions} />
                                                </td>
                                            </tr>
                                        )) :
                                        <tr>
