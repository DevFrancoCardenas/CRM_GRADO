...
# Verifica tipo de contenido y si hay texto
if response.status_code == 200:
    if response.headers.get("Content-Type", "").startswith("application/json") and response.text.strip():
        try:
            data = response.json()
            print("Datos recibidos con exito:", data)

            with open("pensss.json", "w", encoding="utf-8") as file:
                json.dump(data, file, indent=4, ensure_ascii=False)
            print("Los datos han sido guardados en 'pensdd.json'")
        except json.JSONDecodeError:
            print("Error: La respuesta no contiene un JSON. válido.")
    else:
        print("Respuesta vacía o no es JSON:", response.text)
else:
